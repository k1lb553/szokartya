...
def query_me_daily():  
    ...
    new_cards= users_coll.find({"_id": user_id,"decks.known_times": 0}, columns).limit(3)
...
def my_decks():
    ...
    user_document = users_coll.find_one({"_id": user_id})
    user_decks = user_document.get("decks", [])
...
def word_to_card(user_id, side1, side2, decklabel,
    ...
    users_coll.update_one({"_id": user_id}, {"$push": {"decks": new_card}})
    print("added card!!!!")
...
def update_user_decks(user_id, lvl):
    #transforming from ObjectId("asd123") to "asd123"
    user_id = ObjectId(user_id)
